...
//! the `HaltConfig` struct and TOML format are unchanged.

use std::fmt;
use std::str::FromStr;

use crate::{HaltConfig, ProxySettings, SandboxPaths, SandboxSettings};
...
/// Returns the `HaltConfig` fragment for a single profile (platform-aware).
fn profile_config(p: Profile) -> HaltConfig {
...
/// Merges the config fragments for all given profiles into a single `HaltConfig`.
pub fn build_profiles_config(profiles: &[Profile]) -> HaltConfig {
    profiles
        .iter()
        .fold(HaltConfig::default(), |acc, &p| acc.merge(profile_config(p)))
}
...
pub fn is_profile_likely_active(config: &HaltConfig, p: Profile) -> bool {
