...
mod linux {
    ...
    const HALT_BIN: &str = env!("CARGO_BIN_EXE_halt");
    ...
    /// non-system table so the halt proxy can reach the name servers.
    ...
    fn landlock_available() -> bool {
        ...
        *CACHE.get_or_init(|| {
            ...
            // Phase 2: halt check reports ready?
            let api_ok = Command::new(HALT_BIN)
                .arg("check")
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status()
                ...
                .unwrap_or(false);
            if !api_ok {
                println!("NOTE: halt check reports sandbox unavailable — skipping filesystem tests");
                return false;
            }

            // Phase 3: actually enforcing? (behavioral probe)
            let probe = "/etc/halt-landlock-probe";
            let _ = fs::remove_file(probe);
            let _ = Command::new(HALT_BIN)
                .args([
                    "run", "--no-config",
                    "--network", "unrestricted",
                    "--",
                    "bash", "-c",
                    "echo probe > /etc/halt-landlock-probe 2>/dev/null; exit 0",
                ])
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
    ...
    fn run_halt(config: &Path, extra: &[&str], cmd: &[&str]) -> (ExitStatus, String) {
        let out = Command::new(HALT_BIN)
            ...
            .stderr(Stdio::piped())
            .output()
            .unwrap_or_else(|e| panic!("halt spawn: {e}"));
    ...
    fn test_filesystem(agent: &str) {
        ...
        let blocked = format!("/etc/halt-test-{agent}");
